 ...
 * decision). This module reuses the physical/freshness subset of the takeover gate. The class is
 * already locked, so a later feed row temporarily missing its type designator cannot change or
 * invalidate that class.
 *
...
import { checkPhysicalEligibility } from "./eligibility";
...
export function resyncDecision(
  ...
  const eligibility = checkPhysicalEligibility(contact);
  if (!eligibility.eligible) return { ok: false, reason: eligibility.reason };
  // The physical gate has proven the contact is present and non-null; buildSpawnState is safe.
